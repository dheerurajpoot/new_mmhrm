...
export function AdminSettings() {
		...
		try {
			...
			if (response.ok) {
				...
				const newFormData = {
					...
					site_logo:
						data.site_logo &&
						data.site_logo !== "/placeholder-logo.png"
							? data.site_logo
							: "",
	...
	const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (file.size > 2 * 1024 * 1024) {
			toast("Logo size must be less than 2MB");
			return;
		}
		...
		try {
			...
			if (response.ok) {
				...
				if (settingsResponse.ok) {
					toast("Logo uploaded and saved successfully");
					...
					toast("Logo uploaded but failed to save settings");
				}
			} else {
	...
	const resetToDefaults = () => {
		setFormData({
			...
			theme: "light",
			footer_text:
				"Made with ❤️ by Chandu © 2025 HRMS. All rights reserved.",
			...
			<div className='flex items-center justify-between'>
				<div>
					...
					<p className='text-gray-600'>
						Customize your HR management system
					</p>
				</div>
			...
			<form onSubmit={handleSave} className='space-y-6'>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Configure the basic website information
						</CardDescription>
					</CardHeader>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Set up contact details for your organization
						</CardDescription>
					</CardHeader>
					<CardContent className='space-y-4'>
						<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
