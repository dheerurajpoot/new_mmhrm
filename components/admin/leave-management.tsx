...
export function LeaveManagement() {
  ...
  const fetchData = async () => {
    try {
      const [employeesRes, balancesRes, requestsRes, typesRes] = await Promise.all([
        fetch("/api/employees"),
  ...
  const handleTypeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        ...
        const response = await fetch(`/api/leave-types/${editingType.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            name: typeFormData.name,
            description: typeFormData.description,
            max_days_per_year: Number.parseInt(typeFormData.max_days_per_year),
  ...
  const handleDeleteType = async (typeId: string) => {
    const confirmed = window.confirm("Are you sure you want to delete this leave type? This action cannot be undone.")
    if (!confirmed) return
  ...
  const calculateDays = () => {
    if (requestFormData.start_date && requestFormData.end_date) {
      ...
      setRequestFormData({ ...requestFormData, days_requested: diffDays.toString() })
    }
  }

  if (loading && leaveBalances.length === 0) {
    return <div className="flex items-center justify-center h-64">Loading leave data...</div>
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage employee leave balances, requests, and types</p>
        </div>
      </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="days_requested"
                        type="number"
                        value={requestFormData.days_requested}
                        onChange={(e) => setRequestFormData({ ...requestFormData, days_requested: e.target.value })}
                        required
                      />
                    </div>
                  </div>
        ...
        <TabsContent value="balances" className="space-y-4">
          <div className="flex justify-end">
            <Dialog open={isBalanceDialogOpen} onOpenChange={setIsBalanceDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  <DialogTitle>{editingBalance ? "Edit" : "Add"} Leave Balance</DialogTitle>
                  <DialogDescription>
                    {editingBalance ? "Update" : "Set"} leave balance for an employee
                  </DialogDescription>
                </DialogHeader>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setBalanceFormData({ ...balanceFormData, year: Number.parseInt(e.target.value) })
                        }
                        required
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="total_days"
                        type="number"
                        value={balanceFormData.total_days}
                        onChange={(e) => setBalanceFormData({ ...balanceFormData, total_days: e.target.value })}
                        required
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="used_days"
                        type="number"
                        value={balanceFormData.used_days}
                        onChange={(e) => setBalanceFormData({ ...balanceFormData, used_days: e.target.value })}
                      />
                    </div>
                  </div>
                  <div className="flex justify-end space-x-2">
                    ...
                    <Button type="submit" disabled={loading}>
                      {loading ? "Saving..." : editingBalance ? "Update" : "Add"} Balance
                    </Button>
                  </div>
                </form>
              </DialogContent>
            ...
            <Dialog open={isTypeDialogOpen} onOpenChange={setIsTypeDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>{editingType ? "Update the leave type details" : "Create a new leave type for the organization"}</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleTypeSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="max_days"
                        type="number"
                        value={typeFormData.max_days_per_year}
                        onChange={(e) => setTypeFormData({ ...typeFormData, max_days_per_year: e.target.value })}
                        required
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setTypeFormData({ ...typeFormData, carry_forward: e.target.checked })}
                    />
                    <Label htmlFor="carry_forward">Allow carry forward to next year</Label>
                  </div>
                  <div className="flex justify-end space-x-2">
                    ...
                    <Button type="submit" disabled={loading}>
                      {loading ? (editingType ? "Updating..." : "Creating...") : (editingType ? "Update Leave Type" : "Create Leave Type")}
                    </Button>
                  </div>
                </form>
              </DialogContent>
                      ...
                      <TableRow key={type.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            ...
                            <Button variant="outline" size="sm" onClick={() => handleDeleteType(type.id)}>
                              <Trash2 className="h-4 w-4" />
                            </Button>
                          </div>
                        </TableCell>
