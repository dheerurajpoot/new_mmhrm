  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const activities = []
        const recentLeaves = leaves.filter((l: any) => l.status === "pending").slice(0, 4)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">Leave Distribution</h3>
        <div className="flex items-center justify-center">
          <div className="relative w-48 h-48">
            ...
            <div className="absolute inset-0 rounded-full border-8 border-orange-500" style={{ clipPath: "polygon(50% 50%, 100% 50%, 100% 100%, 0% 100%, 0% 50%)" }}></div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-900">Total</p>
                <p className="text-3xl font-bold text-gray-900">{stats.pendingLeaves + stats.overdueApprovals}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            {[1, 2, 3, 4, 5, 6, 7].map((day) => (
              <div key={day} className="flex flex-col items-center space-y-2">
                <div className="flex flex-col space-y-1">
                  <div className="w-8 bg-emerald-300 rounded-t" style={{ height: `${Math.random() * 100 + 20}px` }}></div>
                  <div className="w-8 bg-emerald-500 rounded-t" style={{ height: `${Math.random() * 80 + 10}px` }}></div>
                </div>
              ...
              recentActivity.map((activity, index) => (
                <div key={activity.id} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900">{activity.user || 'Employee'}</p>
