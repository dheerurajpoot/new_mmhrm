		...
		return leaveBalances.map((balance) => {
			...
			const getLeaveTypeConfig = (leaveType: string) => {
				...
				if (type.includes("casual") || type.includes("annual")) {
					...
					type.includes("mensuration") ||
					type.includes("menstrual")
				) {
					return {
						icon: Flower,
						color: "#EC4899",
				...
				<div className='flex flex-col space-y-3 md:flex-row md:items-center md:justify-between md:space-y-0'>
					<div>
						...
						<p className='text-sm md:text-base text-gray-600 mt-1'>
							Track your leave balances and usage
						</p>
					</div>
				...
				{isLeaveDataLoading ? (
					...
					<div className='text-center py-12'>
						...
						<p className='text-gray-600 mb-4'>
							Your leave balances are not yet configured. Please
						...
						{getRealLeaveData()
							...
							.map((leaveType) => {
								...
								const pieData = [
									{
										name: "Used",
										value: leaveType.usedDays,
										color: leaveType.color,
									},
									{
										name: "Remaining",
										value: leaveType.remainingDays,
										color: `${leaveType.color}20`,
									},
								];
								...
								return (
									<Card
										key={leaveType.id}
										className={`relative overflow-hidden border-2 ${leaveType.borderColor} hover:shadow-xl transition-all duration-300 hover:scale-105`}>
										<div
											className={`absolute inset-0 bg-gradient-to-br ${leaveType.bgGradient} opacity-50`}></div>
										<CardContent className='relative px-3 py-0'>
											{/* Header */}
											<div className='flex items-center justify-between mb-4'>
												<div
													className={`p-3 rounded-xl bg-white/80 shadow-sm`}>
													<leaveType.icon
														className={`w-6 h-6 ${leaveType.iconColor}`}
													/>
												</div>
												<div className='text-right'>
													<div
														className={`text-2xl font-bold ${leaveType.textColor}`}>
														{
											...
											{/* Leave Type Name */}
											<h3
												className={`text-lg font-semibold ${leaveType.textColor} mb-3`}>
												{leaveType.name}
											</h3>
												...
												<div className='flex-1 ml-4'>
													<div className='space-y-2'>
														<div className='flex items-center justify-between text-sm'>
															...
															<span
																className={`font-semibold ${leaveType.textColor}`}>
																{
																	leaveType.usedDays
																}
																/
														...
														<div className='text-xs text-gray-500'>
															{leaveType.usagePercentage.toFixed(
																0
															)}
															% used
														</div>
							...
							<div className='flex items-center space-x-4'>
								<Avatar className='w-20 h-20'>
									...
									<AvatarFallback className='bg-gradient-to-br from-blue-400 to-purple-400 text-white font-semibold text-xl'>
										{selectedEmployee.full_name?.charAt(
											0
										) || "E"}
									</AvatarFallback>
								</Avatar>
