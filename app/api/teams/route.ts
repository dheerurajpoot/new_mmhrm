    ...
    const populatedTeams = await Promise.all(
      teams.map(async (team) => {
        ...
        const members = team.member_ids && team.member_ids.length > 0 
          ? await usersCollection.find({ _id: { $in: team.member_ids } }).toArray()
          : []
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !leaderId) {
      return NextResponse.json({ error: "Team name and leader are required" }, { status: 400 })
    }
    ...
    if (memberIds && memberIds.length > 0) {
      const validMemberIds = memberIds.filter(id => ObjectId.isValid(id))
      ...
      const members = await usersCollection.find({ 
        _id: { $in: validMemberIds.map(id => new ObjectId(id)) } 
      }).toArray()
    ...
    const newTeam = {
      ...
      member_ids: (memberIds || []).map(id => new ObjectId(id)),
      created_at: new Date(),
      updated_at: new Date(),
    }
