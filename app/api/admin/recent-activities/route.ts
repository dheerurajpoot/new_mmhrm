  ...
  try {
    ...
    const getEmployeeData = async (employeeId: any): Promise<any> => {
      try {
        ...
        let employee = await profilesCollection.findOne({ _id: employeeId })
        
        if (!employee) {
          const usersCollection = await getUsersCollection()
          const userEmployee = await usersCollection.findOne({ _id: employeeId })
          if (userEmployee) {
    ...
    try {
      ...
      console.log("Found leave requests:", recentLeaveRequests.length)
      ...
      console.log("Found teams:", recentTeams.length)
      ...
      console.log("Found employees:", recentEmployees.length)
      ...
      console.log("Found time entries:", recentTimeEntries.length)

      // Process leave requests (all recent ones, not just pending)
      for (const leaveRequest of recentLeaveRequests) {
        const employee = await getEmployeeData(leaveRequest.employee_id)
        if (employee) {
          activities.push({
            id: `leave-request-${leaveRequest._id}`,
            type: 'leave_request',
            title: 'Leave Request Submitted',
            description: `${employee.full_name || employee.email} submitted a ${leaveRequest.leave_type} request for ${leaveRequest.days_requested || 1} days`,
            details: {
      ...
      for (const leaveRequest of recentLeaveRequests.filter(lr => lr.status !== 'pending' && lr.approved_at)) {
        const employee = await getEmployeeData(leaveRequest.employee_id)
        const approver = leaveRequest.approved_by ? await getEmployeeData(leaveRequest.approved_by) : null
        
        if (employee && approver) {
          activities.push({
            id: `leave-${leaveRequest.status}-${leaveRequest._id}`,
            type: 'leave_approval',
            title: `Leave Request ${leaveRequest.status === 'approved' ? 'Approved' : 'Rejected'}`,
            description: `${approver.full_name || approver.email} ${leaveRequest.status} ${employee.full_name || employee.email}'s ${leaveRequest.leave_type} request`,
            details: {
              leaveType: leaveRequest.leave_type,
              startDate: leaveRequest.start_date,
              endDate: leaveRequest.end_date,
      ...
      for (const team of recentTeams) {
        const leader = team.leader_id ? await getEmployeeData(team.leader_id) : null
        ...
        activities.push({
          ...
          description: leader 
            ? `${leader.full_name || leader.email} created team "${team.name}"`
      ...
      for (const employee of recentEmployees) {
        const employeeCreatedAt = new Date(employee.created_at || employee.updated_at || new Date())
        ...
        if (employeeCreatedAt > sevenDaysAgo) {
          activities.push({
            ...
            description: `${employee.full_name || employee.email} joined as ${employee.role}`,
            details: {
              role: employee.role,
      ...
      console.log("Activity types:", activities.map(a => a.type))
