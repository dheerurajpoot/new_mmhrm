  ...
  try {
    ...
    try {
      ...
      if (user.role === 'admin' || user.role === 'hr') {
        ...
        console.log("Admin/HR: Fetched all leave balances from database:", balances.length)
      } else {
        // Regular employees see only their own leave balances
        balances = allBalances.filter((balance: any) => 
        ...
        console.log("Employee: Fetched leave balances from database for user:", user._id, balances.length)
      }
    } catch (dbError) {
    ...
    console.log("Returning transformed leave balances:", transformedBalances.length, transformedBalances)
